...
async def test_jugar_analisis_correcto(cleanup_db_after_test):
    ...
    with client.websocket_connect(f'ws://localhost:8000/partidas/1/ws?idJugador=1') as ws:
        with patch("api.ws.manager.get_from_message_queue", new_callable=AsyncMock) as mock_get_from_message_queue:
            ...
            response2 = ws.receive_json()
        assert response2 == {"event": "Analisis", "data": ['Lanzallamas', 'Lanzallamas', 'Lanzallamas', 'Lanzallamas']}
    assert response.status_code == 200



async def test_analisis_jugadoresnoadyacentes(cleanup_db_after_test):
    client = TestClient(app)
    crear_datos_partida() 
    async def fake_get_from_message_queue(id_partida, id_objetivo):
        # Simular una respuesta que se esperaría
...
def crear_datos_partida():
    with db_session:
        partida = Partida(nombre="partida", iniciada=True, finalizada=False , maxJug=5, minJug=4,  cantidadVivos=4, sentido=True)
        db.commit()
        ...
        crear_templates_cartas()
        Carta(descartada=False, template_carta="Analisis", partida=partida, jugador=jugador)
        for i in range(4):
            Carta(descartada=False, template_carta="Lanzallamas" , partida=partida, jugador=jugador2)
        commit() 