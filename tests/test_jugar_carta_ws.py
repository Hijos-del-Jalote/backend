...
async def test_jugar_carta_defendido(setup_db_before_test, cleanup_db_after_test):
    ...
    with db_session:
        ...
        j1 = Jugador[ids['idj1']].nombre
        j2 = Jugador[ids['idj2']].nombre
        carta = Carta[ids['idc1']].template_carta.nombre
        carta2 = Carta[ids['idc2']].template_carta.nombre
    ...
    with client.websocket_connect(f'ws://localhost:8000/partidas/1/ws?idJugador=2') as ws:
        with client.websocket_connect(f'ws://localhost:8000/partidas/1/ws/chat?idJugador=2') as wschat:
            ...
            assert logmsg == {'event':"chat_msg",'data':{'isLog':True,'player': None,'msg':f'{j1} jugó {carta} contra {j2}','time':"00:00"}}
            ...
            logmsg = wschat.receive_json()
            assert logmsg == {'event':"chat_msg",'data':{'isLog':True,'player': None,'msg':f'{j2} se defendió con {carta2}','time':"00:00"}}
...
async def test_jugar_carta_no_defendido(setup_db_before_test, cleanup_db_after_test):
    ...
    with client.websocket_connect(f'ws://localhost:8000/partidas/1/ws?idJugador=2') as ws:
        with client.websocket_connect(f'ws://localhost:8000/partidas/1/ws/chat?idJugador=2') as wschat:
            ...
            logmsg = wschat.receive_json()
            assert logmsg == {'event':"chat_msg",'data':{'isLog':True,'player': None,'msg':f'{j2} no se defendió','time':"00:00"}}
